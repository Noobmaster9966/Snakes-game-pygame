...
bgimg = pygame.transform.scale(bgimg,(1200,720)).convert_alpha()

inimg = pygame.image.load("Snake-Pass-4K-Wallpaper-2.jpg")
inimg = pygame.transform.scale(inimg,(1200,720)).convert_alpha()
...
def gameloop():
    ...
    while not exit_game:
        ...
        if(game_over!=True):
            ...
            with open("hiscore.txt", "w") as f:
                f.write(hiscore)
            pygame.mixer.music.load("sfx-defeat1.mp3")
            pygame.mixer.music.play()

            gameWindow.fill(red)
            text_screen("Game Over",black,450,350)
            text_screen("Your Score = "+str(score),black,420,400)
            text_screen("Press Esc to Exit or Enter to Continue !",black,230,500)
            text_screen("Always Remember : Nanu ka saap bhot bada hai",black,170,600)
            pygame.display.update()
